	...
	if (count($_POST) > 0){
		...
		if ($rs = $GLOBALS['db']->Execute($sql)){
			...
			if ($rs->fields['estadoEmpleado']=='Habilitado') {
				...
				echo "<script>alert('Bienvenido ".$rs->fields['nombreEmpleado']." [Nivel: Receptor de Muestras]'); window.location.href='../Views/index.php'</script>";
				}else if ($rs->fields['categoriaEmpleado']=='T') {
				echo "<script>alert('Bienvenido ".$rs->fields['nombreEmpleado']." [Nivel: Tecnico de Laboratorio]'); window.location.href='../Views/index.php'</script>";
		    }}else{
